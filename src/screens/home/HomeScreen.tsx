...
const HomeScreen = () => {
  ...
  const uniqueDrinkCategories = Array.from(
    new Set(DrinkData.map((item) => item.category))
  );
  const uniqueDessertCategories = Array.from(
    new Set(DessertData.map((item) => item.category))
  );

  const generalFoodCategory = FoodData.filter(
    (item) => item.category === "Generelt"
  );

  // SELECTED
  const selectedFoodCategory = FoodData.filter(
    (item) => item.category === selectedFood
  );
  const selectedDrinkCategory = DrinkData.filter(
    (item) => item.category === selectedDrink
  );
  const selectedDessertCategory = DessertData.filter(
    (item) => item.category === selectedDrink
  );
